 ...
 * Intent of this class is to allow expressions like <code>{..., "price": $price, ...}</code>. Note the missing quotes around
 * <code>$price</code>
 ...
 * The alternative by specifying the type of the parameter inside a string like <code> "pizzaPrice": "${float(pizzaPrice)}" </code> is non
 * user friendly. Thus it is better to relax the JsonReader so that any expressions starting with "$" are stored by default as String and
 * will get replaced in the Bson tree with the BsonValue of type same as the actual parameter value that will be provided by the user.
 *
 * NOTE:
 * 1. This class is a copy of org.bson.json.JsonReader because extending it was not possible easily. This means this class will need to be
 * updated whenever pom bson lib is updated. If you store this class under org.bson.json then no need to copy some package private
 * classes like JsonType. Search main change by looking for "*****"
 * 2. Future expressions using function calls like <code>"price": $functionName($paramName)</code> will need to be enclosed within quotes
 * because "(" or "{" are boundary tokens within JsonScanner.
 */
public class RelaxedJsonReader extends AbstractBsonReader {
   ...
   */
  public RelaxedJsonReader(final String json) {
   ...
   * @since 3.11
   */
  public RelaxedJsonReader(final Reader reader) {
    this(new JsonScanner(reader));
  }

  private RelaxedJsonReader(final JsonScanner scanner) {
    super();
    this.scanner = scanner;
    setContext(new RelaxedJsonReader.Context(null, BsonContextType.TOP_LEVEL));
  }
    ...
    switch (token.getType()) {
      ...
      case UNQUOTED_STRING:
        ...
        if ("false".equals(value) || "true".equals(value)) {
          ...
          visitNew();
        // ********** BEGIN CHANGE HERE
        } else if (value.startsWith(BsonParameterEvaluator.DOLLAR)) {
          currentValue = value;
          setCurrentBsonType(BsonType.STRING);
          // ********** END
        } else {
  ...
  protected void doReadStartArray() {
    setContext(new RelaxedJsonReader.Context(getContext(), BsonContextType.ARRAY));
  }

  @Override
  protected void doReadStartDocument() {
    setContext(new RelaxedJsonReader.Context(getContext(), BsonContextType.DOCUMENT));
  }
  ...
  private BsonBinary visitBinDataExtendedJson(final String firstKey) {
    ...
    RelaxedJsonReader.Mark mark = new RelaxedJsonReader.Mark();
  ...
  private BsonRegularExpression visitRegularExpressionExtendedJson(final String firstKey) {
    RelaxedJsonReader.Mark extendedJsonMark = new RelaxedJsonReader.Mark();
  ...
  private void visitJavaScriptExtendedJson() {
    ...
    switch (nextToken.getType()) {
      case COMMA:
        ...
        setContext(new RelaxedJsonReader.Context(getContext(), BsonContextType.SCOPE_DOCUMENT));
        break;
      case END_OBJECT:
        currentValue = code;
  ...
  public BsonReaderMark getMark() {
    return new RelaxedJsonReader.Mark();
  }

  @Override
  protected RelaxedJsonReader.Context getContext() {
    return (RelaxedJsonReader.Context) super.getContext();
  }
  ...
  protected class Mark extends AbstractBsonReader.Mark {
    ...
    protected Mark() {
      super();
      pushedToken = RelaxedJsonReader.this.pushedToken;
      currentValue = RelaxedJsonReader.this.currentValue;
      markPos = RelaxedJsonReader.this.scanner.mark();
    }

    @Override
    public void reset() {
      super.reset();
      RelaxedJsonReader.this.pushedToken = pushedToken;
      RelaxedJsonReader.this.currentValue = currentValue;
      RelaxedJsonReader.this.scanner.reset(markPos);
      RelaxedJsonReader.this.setContext(new RelaxedJsonReader.Context(getParentContext(), getContextType()));
    }
     ...
     */
    public void discard() {
      RelaxedJsonReader.this.scanner.discard(markPos);
    }
  }
  ...
  protected class Context extends AbstractBsonReader.Context {
    ...
    @Override
    protected RelaxedJsonReader.Context getParentContext() {
      return (RelaxedJsonReader.Context) super.getParentContext();
    }
